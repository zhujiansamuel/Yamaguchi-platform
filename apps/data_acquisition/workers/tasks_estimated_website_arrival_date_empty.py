"""
Celery tasks for Estimated Website Arrival Date Empty Worker.

All tasks here will be processed by the estimated_website_arrival_date_empty queue.
"""

import logging
from .celery_estimated_website_arrival_date_empty import app
...
@app.task(
    name='apps.data_acquisition.workers.tasks_estimated_website_arrival_date_empty.process_record',
    bind=True,
    max_retries=3,
...
def process_record(self, **kwargs):
    """
    Process a single Purchasing record with empty estimated_website_arrival_date.
    ...
    logger.info(f"[Task {task_id}] Starting estimated_website_arrival_date_empty processing")

    try:
        worker = EstimatedWebsiteArrivalDateEmptyWorker()
...
@app.task(
    name='apps.data_acquisition.workers.tasks_estimated_website_arrival_date_empty.process_batch',
    bind=True,
)
