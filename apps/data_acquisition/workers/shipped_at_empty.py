"""
Worker for processing Purchasing records with empty shipped_at.
...
class ShippedAtEmptyWorker(BasePlaywrightWorker):
    """
    Worker for extracting shipment information for Purchasing records.

    This worker processes records where confirmed_at is set but shipped_at
    and all subsequent fields are empty, typically to check shipment status.
    """

    QUEUE_NAME = 'shipped_at_empty'
    WORKER_NAME = 'shipped_at_empty_worker'
    ...
    def acquire_record(self) -> Optional['Purchasing']:
        ...
        """
        filter_condition = get_shipped_at_empty_filter()
    ...
    def execute(self, task_data: dict) -> dict:
        """
        Execute the shipment status extraction.
        ...
        try:
            ...
            #
            # 3. Extract shipment status
            # shipped_at = self._extract_shipped_at()
            #
            # 4. Update record with extracted data
            # record.shipped_at = shipped_at
            # record.save(update_fields=['shipped_at'])
