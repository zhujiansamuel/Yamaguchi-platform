"""
Celery tasks for Confirmed At Empty Worker.

All tasks here will be processed by the confirmed_at_empty queue.
"""

import logging
from .celery_confirmed_at_empty import app
...
@app.task(
    name='apps.data_acquisition.workers.tasks_confirmed_at_empty.process_record',
    bind=True,
    max_retries=3,
...
def process_record(self, **kwargs):
    """
    Process a single Purchasing record with empty confirmed_at.
    ...
    logger.info(f"[Task {task_id}] Starting confirmed_at_empty processing")

    try:
        worker = ConfirmedAtEmptyWorker()
...
@app.task(
    name='apps.data_acquisition.workers.tasks_confirmed_at_empty.process_batch',
    bind=True,
)
